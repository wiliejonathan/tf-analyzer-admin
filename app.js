  ...
  async function callApi(command, data = {}) {
    ...
    try {
      ...
      if (err && err.name === "AbortError") {
        throw new Error("API_TIMEOUT_20S — Apps Script tidak merespons. Periksa deployment /exec.");
      }
      throw new Error("API_NETWORK_ERROR — " + (err && err.message ? err.message : String(err)));
    } finally {
